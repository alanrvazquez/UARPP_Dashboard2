...
nontime_vars = c("FieldFissures", "Wbrown_Chalk", "Peak", "Trough", "Breakdown", "Setback", "Final")
time_variables = c("MRY", "HRY")
var_names = c(time_variables, nontime_vars)
...
HRY.data = rice.data %>% select("Cultivar", "FieldFissures", "Wbrown_Chalk")
HRY.data = HRY.data %>%  add_column("MRY" = AUC_variables[,1], "HRY" = AUC_variables[,1]) %>% 
  group_by(Cultivar) %>% 
  summarise_all(mean, na.rm = TRUE)
...
output$ClusterAnalysisALL <- renderPlot({
  res.hclust <- eclust(all.data[,-1], "hclust", k = input$k)
...
output$PCAALL <- renderPlot({
  res.km <- eclust(all.data[,-1], "kmeans", nstart = 25, k = input$k) 
...
output$ClusterAnalysisHRY <- renderPlot({
  res.hclust <- eclust(HRY.data[,-1], "hclust", k = input$k)
  fviz_dend(res.hclust, rect = TRUE) + plot.theme # dendrogam
})
```
...
output$PCAHRY <- renderPlot({
  res.km <- eclust(HRY.data[,-1], "kmeans", nstart = 25, k = input$k) 
  fviz_cluster(res.km) + plot.theme 
})
```
